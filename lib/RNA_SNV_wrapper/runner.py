...
def run_RNAsnp(wild_fa, snp_tags, window=None, plfold_window=None,  mode=None, W_L_ratio=0.80, param_string=''):
    """
    ...
    # Make a shell command line
    cmd = '/home/milad/1software/bin/RNAsnp -f {} -s {} '.format(wild_fa, snp_file.name)
    params = ""
    if mode is not None:
        assert mode in [1, 2, 3]
        params += '-m {} '.format(mode) 
    if plfold_window is not None:
        assert W_L_ratio > 0 and W_L_ratio <= 1
        params  += '-W {} -L {} '.format(plfold_window, int(plfold_window*(W_L_ratio)))
